...
def occu_cop(
    ...
    assert session_duration is None or session_duration.ndim == 2, (
    ...
    if session_duration is not None:
        assert n_sites == session_duration.shape[0], (
            "session_duration must have n_sites rows"
        )
        assert time_periods == session_duration.shape[1], (
    ...
    with numpyro.plate("site", n_sites, dim=-1):
        ...
        with numpyro.plate("time_periods", time_periods, dim=-2):
            ...
            rate_detection = numpyro.deterministic(
                "rate_detection",
                jnp.exp(reg_det(obs_covs) + site_re_det + obs_re),
            )
            ...
            if obs is not None:
                ...
                numpyro.sample(
                    "y", dist.Poisson(jnp.nan_to_num(session_duration * l_det))
                )
